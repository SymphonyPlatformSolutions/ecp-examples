...
export const CleverResearch = (props: AppProps) => {
  ...
  useEffect(() => {
    ...
    openClientChat(researchData.customerRooms.map((c) => c.roomId[props.ecpOrigin])[0]);
  }, [props.ecpOrigin]);
  ...
  const blastReport = () => {
    ...
    (window as any).symphony.sendMessage(payload, {
      mode: 'blast',
      streamIds: researchData.customerRooms.map((c) => c.roomId[props.ecpOrigin]),
            ...
            <div className="table">
              ...
              {researchData.customerRooms.map((customer, index) => (
                <Fragment key={customer.name}>
                  ...
                  <div onClick={() => openClientChat(customer.roomId[props.ecpOrigin])}>
                    <BsChatDotsFill />
                  </div>
                </Fragment>
              ))}
